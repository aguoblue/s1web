...
impl Handler for StaticPageHandler {
    fn handle(request: &HttpRequest) -> HttpResponse {
        println!("StaticPageHandler handle");
        let http::httprequest::Resourse::Path(s) = &request.resource;
...
impl WebServiceHandler {
    fn load_json() -> Vec<OrderStatus> {
        ...
        let orders: Vec<OrderStatus> = serde_json::from_str(&json_contents.unwrap()).unwrap();
        orders
